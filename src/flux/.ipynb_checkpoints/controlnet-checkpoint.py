...
class ControlNetFlux(nn.Module):
    ...
    def __init__(self, params: FluxParams, controlnet_depth=2, patch_size=2, type2id=None):
        super().__init__()
    ...
    def Fourier_filter(self, x, threshold, scale, num_step):
        # FFT
        x_freq = fft.fftn(x, dim=(-2, -1))
        x_freq = fft.fftshift(x_freq, dim=(-2, -1))

        B, C, H, W = x_freq.shape
        mask_low = torch.zeros((B, C, H, W), dtype=torch.bool).to(x.device)

        crow, ccol = H // 2, W // 2
        mask_low[..., crow - threshold:crow + threshold, ccol - threshold:ccol + threshold] = True
        
        x_freq[mask_low] = x_freq[mask_low] * num_step * scale  # low
        x_freq[~mask_low] = x_freq[~mask_low] * min(1 / num_step, 1.5)  # high
